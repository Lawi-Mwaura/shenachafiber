...
export function SolutionPage({ eyebrow, title, description, image, imageAlt, kind, formHeading, serviceAreas, points }: Props) {
  return <><SiteHeader /><main id="main-content" className="canonical-page">
    ...
    <section className="location-list" aria-labelledby="locations-heading"><p className="eyebrow">SUITABLE FOR</p><h2 id="locations-heading">Properties across Kenya.</h2><div>{serviceAreas.map((area, index) => <span key={area}>0{index + 1} / {area}</span>)}</div></section>
    <section className="embedded-form-section" id="quote-form"><Suspense fallback={<p>Preparing the quote form...</p>}><EnquiryForm fixedKind={kind} heading={formHeading} idPrefix={kind.replace("_quote", "")} /></Suspense></section>
  </main><SiteFooter /></>;
}
